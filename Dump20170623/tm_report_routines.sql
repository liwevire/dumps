...
BEGIN
    ...
    declare l_closingBalance decimal(13,3);
    declare l_previousBalance decimal(13,3);
    declare l_extDebit decimal(13,3);
    declare l_extCredit decimal(13,3);
	...
	SELECT SUM(amount) into l_extDebit FROM tm_core.misctransaction where tm_core.misctransaction.transactionType = 0 AND date(tm_core.misctransaction.date) = date(calculationDate);
	SELECT SUM(amount) into l_extCredit FROM tm_core.misctransaction where tm_core.misctransaction.transactionType = 1 AND date(tm_core.misctransaction.date) = date(calculationDate);
	...
	update tm_report.daily set
        ...
        tm_report.daily.miscTransaction = 0,
